...
# Uso de la funcion para extraer datos con el Entity proporcionado	
	# fi <- "2021-02-14"
	source("functions/algoritmo_count.r")
	url1 <- paste("https://mistr.operations.dynamics.com/data/MSAVendInvoicesEntities/$count?",sep="")
	count<-get_count_url(url1,token)

	lote<-10000
	source("functions/genera_url.r")
...
head(u1 %>% mutate(diff = ifelse(UnitPrice == 0 , 0, 1)))